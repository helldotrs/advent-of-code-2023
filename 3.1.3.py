...
def get_digit_map(data):
    ...
    for i, string in enumerate(data):
        mapping = [j for j, char in enumerate(string) if char.isdigit()]
        if mapping:
            result.extend([[i, pos] for pos in mapping])

    return result


def get_characters_map(data):
    result = []

    for i, string in enumerate(data):
        mapping = [j for j, char in enumerate(string) if char != '.']
        if mapping:
            result.extend([[i, pos] for pos in mapping])
