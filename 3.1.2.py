...
input_data = """.............65..................998.........453...................................845..773.........................307....527...........541
...
# dot instance at the end of input_data
input_data.append("." * len(input_data[0]))

# replace everything that is not a digit or a dot with a dot with an x
for i in range(len(input_data)):
    input_data[i] = re.sub(r"[^\d.]", "x", input_data[i])

# for every number (full number, not single digit) in the input that has an x on either side, add it to the total
for i in range(len(input_data)):
    for match in re.finditer(r"\d{2,}", input_data[i]):
        if input_data[i][match.start() - 1] == "x" or input_data[i][match.end()] == "x":
            total += int(match.group())

# for every number (full number, not single digit) in the input that has an x on either side, replace every digit in that number with an x
for i in range(len(input_data)):
    for match in re.finditer(r"\d{2,}", input_data[i]):
        if input_data[i][match.start() - 1] == "x" or input_data[i][match.end()] == "x":
            input_data[i] = input_data[i][:match.start()] + "x" * len(match.group()) + input_data[i][match.end():]
...
# Inside the loop where you check surroundings for each number
for row_index, row in enumerate(input_data):
    ...
    for char_index, char in enumerate(row):
        ...
        # Debugging output
        print(f"Current row: {row}, char: {char}, row_index: {row_index}, char_index: {char_index}, numbers_2d_list: {numbers_2d_list}")
